...
// Blank band between "awarded to" (~y 261) and the top lime rule (~y 356) at native size.
const NAME_CENTER_X = 1024;         // horizontal center of the certificate content
const NAME_BASELINE_Y = 680;        // vertically centered in the name gap
const NAME_MAX_WIDTH = 1400;        // stays clear of the right geometric accent
const NAME_MAX_FONT = 92;           // starting font size (px)
const NAME_MIN_FONT = 40;           // smallest we will shrink to
const NAME_COLOR = 'rgb(4, 75, 55)'; // matches CERTIFICATE title teal
const NAME_FONT = '"Libre Baskerville", "Times New Roman", serif';
...
// Prefer Libre Baskerville when available; fall back to Times New Roman if the CDN font can't load.
async function loadFont() {
  try {
    await document.fonts.load(`700 ${NAME_MAX_FONT}px "Libre Baskerville"`);
    await document.fonts.ready;
  } catch (err) {
    console.warn('Libre Baskerville unavailable, using fallback font.', err);
  }
}
